...
async def analyze_with_visualization(
    ...
    try:
        ...
        try:
            ...
            # Use memory-safe method
            result = audio_analyzer.comprehensive_analysis_memory_safe(file_path)
            analysis_result = result["analysis"]
            features = result["audio_features"]

            # Get visualization data from the memory-safe result (it's already included!)
            visualization_data = result.get("visualization", {})

            logger.info("Using visualization data from memory-safe analysis")
